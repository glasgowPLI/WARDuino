...
class BenchmarkRunner:
    def __init__(self):
        ...
        self.results_dir = os.path.join(self.work_dir, "results-safe")
    ...
    def parse_performance_counters(self, output):
        """Parse WARDuino performance counter data"""
        try:
                ...
                for line in lines:
                    ...
                    if ':' in line:
                        ...
                        if key == "Execution Time":
                            ...
                            perf_data['cfi_checks'] = int(value)
                        elif key == "Total Estimated":
                            perf_data['total_memory_bytes'] = int(value.split()[0])
            
            # Method 2: If new format not found, try old format
            if not perf_data or 'execution_time_cycles' not in perf_data:
                ...
                if 'execution_time_cycles' in perf_data:
                    # Find other performance metrics
                    patterns = {
            ...
            # If execution time successfully parsed, return data
            if 'execution_time_cycles' in perf_data:
                return perf_data
