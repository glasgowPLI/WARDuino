#!/usr/bin/env python3
# run_tests.py - 运行WASM测试并生成安全报告（wasm3版本）
...
class Wasm3TestRunner:
    ...
    def check_wasm3(self):
        ...
        try:
            ...
            if result.returncode == 0 or "wasm3" in result.stdout.lower() or "wasm3" in result.stderr.lower():
                return True, "wasm3可用"
            else:
                # 尝试运行空文件测试
                ...
                result = subprocess.run(
                    [str(self.wasm3_path), str(test_wasm)],
                    capture_output=True, 
                    text=True, 
                    timeout=2
                )
    ...
    def run_wasm3_test(self, wasm_file):
        """运行单个WASM测试"""
        ...
        try:
            # 运行wasm3，捕获所有输出
            result = subprocess.run(
                [str(self.wasm3_path), str(wasm_file)],
    ...
    def run_test_suite(self):
        ...
        for test_name, test_info in self.manifest['tests'].items():
            ...
            # 运行测试
            result = self.run_wasm3_test(wasm_file)
...
def find_wasm3():
    ...
    possible_paths = [
        ...
        "/usr/local/bin/wasm3",
        "./wasm3",
        "../wasm3/build-emu/wasm3",
        "../wasm3/build-secure/wasm3"
    ]
...
def main():
    ...
    if not wasm3_path:
        print("❌ 未找到wasm3可执行文件")
        print("请确保wasm3已编译且在以下路径之一:")
        print(" - ../wasm3/build/wasm3")
        print(" - ../wasm3/wasm3")
        print(" - 当前目录的wasm3")
        print(" - /usr/local/bin/wasm3")
        return
